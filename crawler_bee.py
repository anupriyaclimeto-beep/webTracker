"""
crawler_bee.py — BEE RCO Portal Crawler
URL: https://beeindia.gov.in/show_content.php?lang=1&level=1&ls_id=119&lid=291

This crawler visits the BEE India Renewable Consumption Obligations page, 
takes a screenshot, and runs diffs.
"""
...
PORTAL_NAME = "BEE RCO"
HOME_URL    = "https://beeindia.gov.in/show_content.php?lang=1&level=1&ls_id=119&lid=291"

async def crawl_bee_portal(portal_config, mode="full"):
    init_db()
    ...
    async with async_playwright() as p:
        ...
        page_key = "bee_rco_home"
        
        old_data = get_baseline(PORTAL_NAME, page_key)
        
        from datetime import datetime
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        safe_url = HOME_URL.replace("https://", "").replace("http://", "").replace("/", "_").replace("?", "_").replace("&", "_").replace("=", "_")
...
if __name__ == "__main__":
    ...
    asyncio.run(crawl_bee_portal(p_config))
