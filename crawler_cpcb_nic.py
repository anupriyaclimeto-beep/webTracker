...
async def save_snapshot(page, key: str, screenshot_bytes: bytes) -> dict:
    ...
    remote_shot = upload_to_cloudinary(str(screenshot_path), resource_type="image")
    remote_html = upload_to_cloudinary(str(html_path), resource_type="raw")
...
async def crawl_cpcb_nic_portal(portal_config: dict, limit: int | None = None, mode="full"):
